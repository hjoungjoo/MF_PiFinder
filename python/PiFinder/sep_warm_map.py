...
Usage (device, any mix of dump directories)::

    python -m PiFinder.sep_warm_map ~/PiFinder_data/captures
    python -m PiFinder.sep_warm_map /dev/shm/pifinder/captures --dry-run

Inputs are stage-dump rasters, which are already in solver_raw
orientation (profile rot90 applied):
...
def _load_frames(dump_dirs):
    ...
    for root in dump_dirs:
        for pattern, kind in (
            ...
            for path in sorted(Path(root).glob(f"stages_*/{pattern}")):
