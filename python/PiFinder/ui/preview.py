...
# MF: processed-frame background level (0-255) above which the Image view
# switches to rendering the raw sensor frame directly. The processed preview
# chain assumes a dark sky; daytime or saturated frames wash it out, while
# the 12-bit raw frame still holds a usable scene for daytime alignment.
...
class UIPreview(GuideKeyMixin, UIModule):
    ...
    def _build_marking_menu() -> MarkingMenu:
        """Keep Help and MF Gain; exposure is adjusted with UP/DOWN.
    ...
    def inactive(self):
        """Release once, including MF guide key cleanup."""
        super().inactive()
        if not self._exposure_hold_active:
            return
        self._exposure_hold_active = False
        self.command_queues["camera"].put(f"focus_end:{self._focus_hold_token}")

    def covered(self):
        """Release when another screen covers or discards Focus."""
        self.inactive()
    ...
    def _begin_exposure_hold(self) -> None:
        ...
        self._focus_hold_token = uuid.uuid4().hex
        self._held_exposure_us = self._entry_exposure()
        self._exposure_hold_active = True
        self.command_queues["camera"].put(
            f"focus_begin:{self._focus_hold_token}:{self._held_exposure_us}"
        )
    ...
    def _apply_hold_exposure(self, exposure_us: int) -> None:
        ...
        self._exposure_hold_active = True
        self.command_queues["camera"].put(
            f"focus_set:{self._focus_hold_token}:{self._held_exposure_us}"
        )
    ...
    def _orient_camera_image(self, image_obj: Image.Image) -> Image.Image:
        """MF: rotate a raw sensor frame to match the processed preview."""
    ...
    def _render_raw_bright_frame(self) -> Optional[Image.Image]:
        """MF: render a daylit/saturated scene from the raw sensor frame.

        The processed preview is exposed and stretched for a dark sky, so a
    ...
    def update(self, force: bool = False):
        ...
        if last_image_time > self.last_update:
            ...
            if self.display_mode == DISPLAY_STARS:
                ...
                rendered = None
                if float(np.median(raw_np)) >= STRETCH_BRIGHT_BACKGROUND:
                    rendered = self._render_raw_bright_frame()
                if rendered is None:
                    rendered = self._render_image_frame(raw_image)
                self.screen.paste(rendered)
