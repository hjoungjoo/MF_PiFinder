...
def run_messages(messages, clock=None):
    ...
    # MF: sats is [seen, used, in_view, top_cno]; reset all four between runs
    gps_ubx.sats[0] = 0
    gps_ubx.sats[1] = 0
    gps_ubx.sats[2] = 0
    gps_ubx.sats[3] = ()
    ...
    # MF publishes 4-tuples (seen, used, in_view, top_cno); compare (seen, used)
    return [content[:2] for name, content in gps_queue.items if name == "satellites"]
