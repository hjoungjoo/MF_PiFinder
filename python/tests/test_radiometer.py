...
def test_sky_colour_is_sampled_on_the_right_mosaic_sites(height, width):
    ...
    """
    # MF: lift the mono override -- this test exercises the sampler
    profile = replace(get_camera_profile("imx462"), mono=False)
...
def test_rotated_or_odd_cropped_profiles_refuse_to_report_colour():
    ...
    # MF: the shipped imx462 is measured mono, so the colour baseline for
    # this test needs the mono override lifted (the label/rotation/crop
    # invariants under test are orthogonal to it).
    profile = replace(get_camera_profile("imx462"), mono=False)
...
def test_shipped_colour_profiles_hold_the_phase_invariants():
    ...
    MF: of upstream's three "colour" profiles, this fork ships imx462 and
...
@pytest.mark.unit
def test_measured_mono_imx462_keeps_the_constant_zero_point():
    """MF regression pin for the #560 port.

    The shipped imx462 carries a SRGGB12 driver label but measures as true
    mono (CameraProfile.mono, §6.4): its R/G pins at ~1.00 under every sky.
    Upstream's colour model reads that as a dark-site spectrum and would
    shift the published SQM by ~+0.74 mag, permanently, in every sky —
    worst under the light pollution this fork targets. So the mono flag
    must (a) keep colour fields out of the sample and (b) leave the
    effective zero point at the profile constant.
    """
    profile = get_camera_profile("imx462")
    assert profile.mono

    frame = np.full((256, 256), 300, dtype=np.uint16)
    sample = collect_radiometer_sample(frame, profile, 1.0, sequence=1, captured_at=0.0)
    assert "background_red" not in sample
    assert "background_green" not in sample

    value, details = radiometric_sqm(sample, profile)
    assert value is not None
    assert details["radiometric_zero_point_effective"] == profile.radiometric_zero_point
    assert "sky_red_over_green" not in details
