...
def test_gps_time_sync_settings_menu_entries_exist():
    ...
    entries = {
        ...
        for node in _iter_menu_nodes(menu_structure.pifinder_menu)
        if node.get("config_option") in expected_options | removed_options
    }
...
def test_gps_time_sync_status_summary_lines():
    ...
    assert not any(line.startswith("NTP") for line in lines)
    assert not any(line.startswith("Sys") for line in lines)
    assert not any(line.startswith("PPS") for line in lines)
...
def test_gps_time_sync_status_detail_lines_include_helper_results():
    ...
    status = {
        "state": "stable",
        "message": "Selected Chrony time source",
        "clock_manager": "chrony",
        "selected": {
            "source": "Chrony",
    ...
    assert "Clock: chrony" in lines
    assert "Selected: Chrony" in lines
    ...
    assert not any(line.startswith("NTP") for line in lines)
    assert not any(line.startswith("Sys re") for line in lines)
