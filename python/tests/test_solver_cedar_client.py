...
def test_extract_centroids_falls_back_when_segment_vanishes(caplog):
    ...
    try:
        # Hand the client a real segment under a test-only name, then delete
        # its name out from under it, exactly as logind's RemoveIPC does.
        # (The inherited _alloc_shmem hardcodes /cedar_detect_image, which on
        # a PiFinder unit is the LIVE solver's segment — the test must never
        # touch that, so the segment is created here instead.)
        client._shmem = shared_memory.SharedMemory(
            "/cedar_detect_image_pftest", create=True, size=image.size
        )
        client._shmem_size = image.size
