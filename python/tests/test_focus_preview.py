...
def _hold_preview(*, camera_exp="auto", exposure_time=437_000):
    ...
    preview.command_queues = {
        "camera": SimpleNamespace(
            put=lambda command: preview.camera_commands.append(
                command.replace(preview._focus_hold_token, "token")
            )
        )
    }
    return preview
...
def test_entering_focus_holds_the_exposure_auto_exposure_settled_on():
    ...
    assert preview.camera_commands == ["focus_begin:token:437000"]
...
def test_entering_focus_prefers_a_chosen_exposure_over_stale_frame_metadata():
    ...
    assert preview.camera_commands == ["focus_begin:token:200000"]
...
def test_entering_focus_without_a_known_exposure_falls_back_to_the_default():
    ...
    assert preview.camera_commands == [f"focus_begin:token:{FOCUS_DEFAULT_EXPOSURE}"]
...
def test_leaving_focus_hands_auto_exposure_back_exactly_once():
    ...
    assert preview.camera_commands == ["focus_end:token"]
...
def test_leaving_focus_restores_a_manual_exposure_without_rewriting_it():
    ...
    # Release camera-owned state without persisting camera_exp.
    assert preview.camera_commands == ["focus_end:token"]
...
def test_arrow_keys_nudge_the_held_exposure_along_the_ladder():
    ...
    assert preview.camera_commands == [
        "focus_set:token:400000",
        "focus_set:token:200000",
        "focus_set:token:400000",
    ]
...
def test_focus_screen_offers_no_exposure_jump_but_keeps_its_help():
    ...
    assert jumps == [None, None, "camera_gain", None]
...
def test_covering_focus_with_new_or_cached_menu_releases_hold(stateful):
    from PiFinder.ui.base import UIModule
    from PiFinder.ui.menu_manager import MenuManager

    class NextScreen(UIModule):
    ...
    preview = _hold_preview(camera_exp="auto_star")
    preview._begin_exposure_hold()
    stops = []
    preview._guide_stop_motion_if_active = lambda: stops.append(True)
    manager = object.__new__(MenuManager)
    manager.stack = [preview]
    for name in (
        "display_class",
        "camera_image",
        "shared_state",
    ...
    manager.config_object = SimpleNamespace(get_option=lambda *args: 0)
    item = {"class": NextScreen}
    if stateful:
        item["state"] = NextScreen()

    manager.add_to_stack(item)

    assert len(manager.stack) == 2
    assert preview.camera_commands == ["focus_begin:token:437000", "focus_end:token"]
    assert not preview._exposure_hold_active
    assert stops  # MF guide cleanup still runs.


@pytest.mark.unit
def test_recent_stack_shortcut_releases_discarded_focus():
    from PiFinder.ui.menu_manager import MenuManager

    preview = _hold_preview(camera_exp="auto_star")
    preview.item_definition = {"label": "focus"}
    preview._begin_exposure_hold()
    recent = SimpleNamespace(item_definition={"label": "recent"}, active=lambda: None)
    manager = object.__new__(MenuManager)
    manager.stack = [recent, preview]

    manager.jump_to_label("recent")

    assert manager.stack == [recent]
    assert preview.camera_commands[-1] == "focus_end:token"
    assert not preview._exposure_hold_active
...
def test_raw_bright_frame_averages_the_bayer_checker():
    ...
    assert rendered.size == (128, 115)  # Entire scene fits above the status bar.
...
def test_image_mode_wires_the_bright_raw_branch():
    ...
    init_src = inspect.getsource(UIPreview._build_marking_menu)
