...
| [cedar_fullframe_primary_plan](mf_dev/mf_cedar_fullframe_primary_plan_ko.md) | ko | **구현 완료**(2026-08-03) | cedar 풀프레임 1차 경로 전환 준비 계획 — `solver_cedar_fullframe`/게이트/중앙 우선 캐스케이드. 실측은 [야간 리포트](mf_report/mf_solver_fullframe_field_test_20260803_ko.md). |
...
| [camera_mono_color_plan](mf_dev/mf_camera_mono_color_plan_ko.md) | ko | **plan(구현 전)** | IMX296/IMX462 mono·color 변형을 설정(Camera Type 5항목 + `camera_variant`)으로 선택 — 파생 프로파일 이름으로 전 프로세스 전파, dtoverlay 불변. |
