...
| [auto_exposure_plan](mf_auto_exposure_plan_ko.md) | ko | **implemented (field validation pending)** | Detected-star-count controller design+implementation — existing behavior kept, selected via the Camera Exp menu's "Star" item (`camera_exp=auto_star`). ADR 0020. |
