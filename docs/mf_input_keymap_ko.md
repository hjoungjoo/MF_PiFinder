...
활성화됩니다. 연속 조그는 키보드 문자(`q w e / a s d / z x c`, `s` = 정지)에도 있고,
`+`/`-`는 슬루 속도(빠르기)입니다.
...
| `0-9` | 마운트 ON이면 **공통 마운트 맵**(2/4/6/8 누른 동안 이동, 0=정지·7=Sync; 5/GoTo는 선택 객체 없어 미사용), 아니면 무효 |
| 문자 / `+` `-` | 마운트 ON이면 INDI 방향 조그 / 슬루 속도, 아니면 무효 |

메뉴는 지금 그대로 동작하되, 숫자 키만 8방향 방향 가이드 대신 공통 마운트 맵
(누른 동안 이동 방향키 + 0/5/7)을 사용.
...
| `0-9` (키패드) | **공통 마운트 맵**: 2/4/6/8 = 남/서/동/북 **누른 동안 이동**, 5=**GoTo** 이 객체, 7=**Sync**, 0=정지 (1/3/9 미사용) |
...
| G2 객체 상세 | 공통 마운트 맵(누른 동안 이동 + 0/5/7) | 연속 조그 | 예 |
...
부수적 마운트 접근(G1 마운트 ON 메뉴, G2 Object Details, G7 상태)은 공통 마운트 맵
(2/4/6/8 누른 동안 남/서/동/북 이동, 0=정지 7=Sync; 5=GoTo는 객체가 선택된 Object
Details에서만; 1/3/9 미사용)을 사용하고,
연속 방향 조그는 키보드 문자에도 있음. 전용 INDI 조그 컨트롤러(G5)는 자체 방향 조그
체계를 유지.
...
2. **공통 마운트 숫자 키**(완료): *부수적* 마운트 접근(Object Details, 마운트 ON
   ...
   **제거** — 누른 만큼 이동하고, 기동 시 이미 init/sync됨. `UIModule`의 공유
   ...
   `UIObjectDetails`가 사용. 전용 INDI 조그 컨트롤러(G5)는 자체 방향 조그 체계를 유지.
