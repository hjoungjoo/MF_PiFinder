...
`skysafari_indi_goto`가 켜져 있으면 SkySafari GoTo뿐 아니라 SkySafari
Align/Sync도 INDI/OnStep 쪽으로 전달한다. `skysafari_indi_sync`는 GoTo 전송은
끄고 Align/Sync만 전달하고 싶을 때 사용하는 별도 허용 옵션이다.
