...
1. **FF 빠른 포기** (우선): `_solve_cedar_fullframe`의 solve_timeout을
   1000→300ms로 단축(상수 `CEDAR_FF_SOLVE_TIMEOUT_MS`). 성공 솔브는
   9–26ms(실측)라 손실 없음; 절벽 위 시도율 0.4→~1.4Hz 회복으로 SEP
   구제 기회 복원. (대안으로 "검출<N이면 매칭 스킵"도 있으나 타임아웃
   단축이 더 단순·안전)
2. **SEP 폴백 백오프 재측정**: #1 적용 후 절벽 위 구제율 재측정. 필요시
   "SEP 검출 ≥ 12일 때 백오프 면제" 검토.
3. **AE 0별 복구 사다리 하향 단 추가** (본 작업과 독립, 2026-08-03 박명
   실측): 사다리 [400k,800k,1M,200k]µs의 최소 단이 400ms라 밝은 하늘
   (적정 ~180ms)에서 전 단 포화 → 무한 순환. 100k/150k 단 추가 또는
   현재 앵커 기준 양방향 사다리로 개편. 소유: ax/camera.md 체계.
4. **솔브 경로 필드 발행**: SolveDiagnostics에 path(cedar_ff/cedar_512/
   sep) 추가 — 현재는 Matches>Centroids 휴리스틱으로만 구분 가능.
   현장 진단·통계 단순화.
