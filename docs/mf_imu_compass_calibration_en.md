...
Set `Settings > IMU Compass > On` to use BNO055 NDOF mode. This includes the magnetometer and can improve absolute heading stability, but it is sensitive to nearby metal, current, magnets, and calibration quality.

## Automatic Calibration

1. Set `Settings > IMU Compass > On`.
2. Restart PiFinder.
...
- If NDOF is unstable, set `Settings > IMU Compass > Off` to return to the legacy IMUPLUS behavior.
