...
A resync storm can yield markers far faster than real messages arrive, so the publisher caps how often it forwards events onto `gps_queue`; the cap is a rate limit on *reporting*, not on parsing. Only the event's name is forwarded — the main process stamps receipt with its own monotonic clock, and the row measures age from that receipt, independently of GPS wall-clock adjustments.
