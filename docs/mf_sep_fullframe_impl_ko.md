...
### 6.7 3차 야간 가동 — 어두운 하늘 검증 (2026-07-28 심야)
...
- **미확정(잔여)**: ① 실망원경 정렬~푸시투 정밀 검증, ② cedar/SEP 역할
  분담 최종안(§7-5 — §6.7 3자 비교로 판정 자료는 완비: 후보안 "cedar
  풀프레임을 2차, SEP을 3차"까지 포함해 ADR로 결정), ③ 계절/온도 변화에
  따른 웜픽셀 맵 재생성 주기.
