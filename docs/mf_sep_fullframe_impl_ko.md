...
>
> 상태: **1차 튜닝 확정 — 맑은 하늘 재검증 대기** (2026-07-28)
...
- **확정**: 좌표 정합 무결(§3.2), 목표 조건에서 SEP이 주력 솔버(§6.5,
  정확도 11–16″), 하이브리드 정렬 동작(간이 확인), 방어선(오솔브 0),
  웜픽셀 마스크(47위치), **1차 튜닝 세트 = σ4.5 + 게이트 5 + 형태·클러스터
  게이트** (2026-07-28 야간 확정 — 동일 프레임 짝비교로 솔브 무손실·순도
  91% 검증), 저장 정책.
- **미확정(재검증 대기)**: ① 맑은 하늘에서 1차 튜닝 세트의 장시간 솔브율
  (이날 밤은 구름 우세라 σ3.5 맑은 창 88~98%와의 등가성 미확인 — CSV
  자동 축적으로 판정), ② 자정 어두운 하늘의 웜픽셀 마스크 실동작
  (박명에선 masked ~0), ③ cedar 980² 풀해상도의 실별 이득, ④ 실망원경
  정렬 정밀 검증.
...
3. 맑은(부분 맑음) 밤 세션 — CSV·코퍼스 자동 축적 (조작 불필요),
   자정 어두운 하늘에서 웜픽셀 마스크 실전 재검증(§6.5)
4. 오프라인 벤치: 별 있는 코퍼스에 σ 스윕 + **cedar 980² 풀해상도(hot
   on)의 실별 검출 이득 판정** (§6.3에서 미판정으로 남은 항목)
5. 최종 방안 결정: 폴백 상시화 여부(§6.5 실측이 강하게 지지), σ 기본값,
   cedar와의 역할 분담 → 결정 시 ADR로 승격, 본 문서는 구현 기록으로 유지
